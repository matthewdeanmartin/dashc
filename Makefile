...
test: clean uv.lock install_plugins
	...
	# $(VENV) pytest --doctest-modules dashc
	# $(VENV) python -m unittest discover
	$(VENV) pytest tests -vv -n 2 --cov=dashc --cov-report=html --cov-fail-under 40 --cov-branch --cov-report=xml --junitxml=junit.xml -o junit_family=legacy --timeout=5 --session-timeout=600
...
.build_history/black: .build_history .build_history/isort $(FILES)
	...
	$(VENV) black dashc # --exclude .venv
	$(VENV) black tests # --exclude .venv
	$(VENV) git2md dashc --ignore __init__.py __pycache__ --output SOURCE.md
...
.build_history/pylint: .build_history .build_history/isort .build_history/black $(FILES)
	...
	$(VENV) ruff --fix
	$(VENV) pylint dashc --fail-under 9.8
...
mypy:
	...
	$(VENV) mypy dashc --ignore-missing-imports --check-untyped-defs


check_docs:
	$(VENV) interrogate dashc --verbose
	$(VENV) pydoctest --config .pydoctest.json | grep -v "__init__" | grep -v "__main__" | grep -v "Unable to parse"

make_docs:
	pdoc dashc --html -o docs --force
...
check_spelling:
	$(VENV) pylint dashc --enable C0402 --rcfile=.pylintrc_spell
	$(VENV) pylint docs --enable C0402 --rcfile=.pylintrc_spell
	$(VENV) codespell README.md --ignore-words=private_dictionary.txt
	$(VENV) codespell dashc --ignore-words=private_dictionary.txt
...
#audit:
#	# $(VENV) python -m dashc audit
#	$(VENV) tool_audit single dashc --version=">=2.0.0"
...
core_all_tests:
	./scripts/exercise_core_all.sh dashc "compile --in examples/compile/src --out examples/compile/out --dry-run"
	uv sync --all-extras
...
dont-be-lazy:
	...
	@uv run dont_be_lazy --root . --no-color scan dashc --no-config-suppressions || true

.PHONY: pydoc-docs
pydoc-docs:
	@uv run pydoc_fork dashc -o ./pydoc/
