...
# readonly SCRIPT_NS="dashc_proc"
readonly SRC_DIR="./dashc"
readonly TMP_ROOT="/tmp"
readonly TMP_DIR="${TMP_ROOT}/dashc"
readonly OUTPUT_MD_BASENAME="dashc.flat.md"

# Copy directory safely
dashc_proc::copy_to_tmp() {
  if [[ ! -d "${SRC_DIR}" ]]; then
...
# Run strip-docs for each subdirectory of tmp/dashc
dashc_proc::run_strip_docs() {
  local dir
...
# in the generated Markdown (dashc.flat.md). Other code fences remain untouched.
dashc_proc::clean_flat_md_trailing_double_hash() {
  local output_md
...
main() {
  dashc_proc::copy_to_tmp
  dashc_proc::run_strip_docs
  dashc_proc::confirm "Run coderoller-flatten-repo?"
  dashc_proc::flatten_repo
  # Post-process the flattened Markdown to remove trailing '##' in Python code blocks only
  dashc_proc::clean_flat_md_trailing_double_hash
  dashc_proc::confirm "Delete temporary files in '${TMP_DIR}'?"
  dashc_proc::cleanup_tmp
}
